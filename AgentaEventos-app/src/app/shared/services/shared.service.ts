...
/* Interfaces */
import { IConsoleParser } from '../interfaces/consoleParser';
...
export class SharedService {
  ...
  checkElementHeight(elementId: string): number {
  ...
  checkIfDate(element: string): boolean {
    return (
      element.length >= 6 &&
      !isNaN(new Date(element).getTime()) &&
      isValid(parseISO(element))
    );
  }
  ...
  getHTML(type: string): string {
    switch (type) {
      ...
      case 'create':
        return `
        <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="currentColor" class="bi bi-file-earmark-plus-fill" viewBox="0 0 16 16">
  ...
  checkRole(
    ...
    switch (role2Check) {
      case 'basic':
        return true;
      case 'premium':
        return ['premium', 'special'].includes(roleType);
      case 'special':
      ...
      case 'operador':
        return (
          ['premium', 'special'].includes(roleType) &&
          ['operador', 'administrador', 'técnico'].includes(generalRole)
        );
      case 'administrador':
        return (
          ['special'].includes(roleType) &&
          ['administrador', 'técnico'].includes(generalRole)
        );
      case 'técnico':
        return ['special'].includes(roleType) && generalRole === 'técnico';
      default:
        return false;
    }
  }
